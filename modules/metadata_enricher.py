...
class MetadataEnricher:
    ...
    async def enrich_entries(self, entries: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        async with aiohttp.ClientSession(headers=self.headers) as session:
            ...
            enriched_entries = await self._gather_with_concurrency(tasks, max_concurrent=3)
        
        return enriched_entries
    
    async def _gather_with_concurrency(self, tasks: List, max_concurrent: int = 3) -> List:
    ...
    async def _enrich_single_entry(self, session: aiohttp.ClientSession, 
                                   entry: Dict[str, Any], index: int, total: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.verbose >= 1:
                print(f"[TIMEOUT] Entry {index} timed out after 2 minutes: {entry.get('title', 'Unknown')[:60]}...")
    ...
    async def _enrich_single_entry_impl(self, session: aiohttp.ClientSession, 
                                        entry: Dict[str, Any], index: int, total: int) -> Dict[str, Any]:
        ...
        if cached_metadata:
            if self.verbose >= 2:
                ...
                if cached_metadata.get('keywords'):
                    print(f"  - Cached keywords: {len(cached_metadata['keywords'])} found")
        ...
        for strategy_name, strategy_func in strategies:
            try:
                ...
                success = await strategy_func(session, entry, metadata)
                
                if success and (metadata['doi'] or metadata['abstract'] or metadata['keywords']):
                    if self.verbose >= 2:
                        ...
                        if metadata.get('keywords'):
                            print(f"  - Found keywords: {', '.join(metadata['keywords'][:5])}")
                            if len(metadata['keywords']) > 5:
                                print(f"    ... and {len(metadata['keywords']) - 5} more")
                    break
        ...
        if metadata.get('doi'):
            ...
            if not current_url or 'doi.org' in current_url or current_url == f"https://doi.org/{metadata['doi']}":
                ...
                if converted_url and converted_url != current_url:
                    ...
                    if self.verbose >= 2:
                        print(f"[ENRICH] Converted DOI to publisher URL: {converted_url}")
                    
            # Always try to fetch abstract and keywords from publisher page if we don't have them
            if metadata.get('publisher_url') and (not metadata.get('abstract') or not metadata.get('keywords')):
        ...
        if not metadata.get('keywords') and self.llm_enricher and self.llm_enricher.enabled:
            title = entry.get('title', '')
            abstract = metadata.get('abstract', '') or entry.get('abstract', '')
            
            if title or abstract:
                if self.verbose >= 2:
                    print("[ENRICH] No keywords found, attempting to generate with LLM...")
                
                generated_keywords = await self.llm_enricher.generate_keywords(title, abstract)
    ...
    async def _enrich_from_url(self, session: aiohttp.ClientSession, 
                              entry: Dict[str, Any], metadata: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            async with session.get(url, timeout=self.config.get('preferences.timeout', 30)) as response:
                ...
                doi_meta = soup.find('meta', {'name': 'citation_doi'}) or \
                          soup.find('meta', {'name': 'DC.Identifier', 'scheme': 'doi'})
                if doi_meta:
    ...
    async def _enrich_from_crossref(self, session: aiohttp.ClientSession, 
                                   entry: Dict[str, Any], metadata: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            year = entry.get('year')
            if year:
                query_params['filter'] = query_params.get('filter', '') + f',from-pub-date:{year},until-pub-date:{year}'
            
            # Search CrossRef using requests (synchronous, but in async context)
            import requests
            email = self.config.get('api_keys.crossref_email', '')
            headers = {'User-Agent': self.headers.get('User-Agent', 'Academic Metadata Extractor')}
            if email:
            ...
            for work in items[:5]:  # Check first 5 results
                ...
                if book_title and container_titles:
                    ...
                    container_match = self._calculate_similarity(book_title.lower(), container_title) > 0.7
                
                # Check if titles match (either chapter title or we have a container match)
                title_match = self._calculate_similarity(title.lower(), work_title) > 0.8
            ...
            if not items and book_title:
                ...
                if response.status_code == 200:
                    ...
                    for work in items[:2]:
                        ...
                        if container_titles:
                            container_title = container_titles[0].lower()
                            if self._calculate_similarity(book_title.lower(), container_title) > 0.8:
                                ...
                                if book_doi and book_doi.startswith('10.1007/'):
                                    ...
                                    if self.verbose >= 2:
                                        print(f"[ENRICH] Found book DOI: {book_doi}, searching for chapter...")
    ...
    async def _enrich_from_publisher(self, session: aiohttp.ClientSession, 
                                     entry: Dict[str, Any], metadata: Dict[str, Any]) -> bool:
    ...
    async def _enrich_from_springer(self, session: aiohttp.ClientSession, 
                                   entry: Dict[str, Any], metadata: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if url and 'springer.com' in url:
                ...
                if doi_match:
                    ...
                    metadata['publisher_url'] = f"https://link.springer.com/chapter/{metadata['doi']}"
                    if self.verbose >= 2:
            ...
            search_url = f"https://link.springer.com/search?query={quote_plus(search_query)}"
            ...
            async with session.get(search_url, timeout=self.config.get('preferences.timeout', 30)) as response:
                if response.status != 200:
                    return False
                ...
                results = soup.find_all('a', {'class': 'title'}) or \
                         soup.find_all('h3', class_=re.compile('result-item-title'))
                
                if not results and self.verbose >= 3:
                    print("[ENRICH] No results found with standard selectors, trying alternative")
                ...
                for result in results[:5]:  # Check first 5 results
                    ...
                    if book_title:
                        ...
                        parent_elem = result.find_parent('div', class_=re.compile('result'))
                        if parent_elem:
                    ...
                    if title_sim > 0.7 or book_match:
                        ...
                        if href:
                            chapter_url = href if href.startswith('http') else f'https://link.springer.com{href}'
                            ...
                            if doi_match:
                                ...
                                if self.verbose >= 2:
                                    print(f"[ENRICH] Found Springer chapter DOI: {metadata['doi']}")
                                
                                # Always fetch the chapter page for abstract and keywords
                                if not metadata.get('abstract') or not metadata.get('keywords'):
                                    try:
                                        async with session.get(chapter_url, timeout=self.config.get('preferences.timeout', 30)) as chapter_response:
                                            if chapter_response.status == 200:
                                                chapter_html = await chapter_response.text()
                                                self._parse_springer_chapter(chapter_html, metadata)
                                                # Set publisher URL to the chapter URL
                                                metadata['publisher_url'] = chapter_url
                                    except Exception as e:
                                        if self.verbose >= 3:
                                            print(f"[ENRICH] Failed to fetch chapter page: {e}")
                ...
                if not found_match and book_title and not title_sim > 0.5:
                    ...
                    async with session.get(book_search_url, timeout=self.config.get('preferences.timeout', 30)) as book_response:
                        if book_response.status == 200:
                            book_html = await book_response.text()
                            book_soup = BeautifulSoup(book_html, 'html.parser')
                            
                            # Find book results
                            book_results = book_soup.find_all('a', {'class': 'title'})[:3]
                            
                            for book_result in book_results:
                                book_result_title = book_result.get_text(strip=True).lower()
                                if self._calculate_similarity(book_title.lower(), book_result_title) > 0.8:
                                    book_href = book_result.get('href', '')
                                    if book_href:
                                        # Extract book DOI and try to construct chapter DOI
                                        book_doi_match = re.search(r'10\.1007/(978-[0-9-]+)', book_href)
                                        if book_doi_match:
                                            book_isbn = book_doi_match.group(1)
                                            if self.verbose >= 2:
                                                print(f"[ENRICH] Found book ISBN: {book_isbn}, inferring chapter DOI...")
    ...
    def _parse_springer_chapter(self, html: str, metadata: Dict[str, Any]) -> bool:
        ...
        doi_elem = soup.find('span', {'class': 'bibliographic-information__value', 
                                     'data-test': 'doi-value'})
        if not doi_elem:
            # Try meta tag
        ...
        if not abstract_elem:
            ...
            abstract_elem = soup.find('div', {'class': 'chapter-abstract-container'}) or \
                           soup.find('section', {'id': 'Abs1'}) or \
                           soup.find('div', {'class': 'AbstractSection'})
        
        if abstract_elem:
            # Try to find paragraphs first
            abstract_paras = abstract_elem.find_all('p')
            if abstract_paras:
                abstract_text = ' '.join([p.get_text(strip=True) for p in abstract_paras])
            else:
        ...
        if keywords_section:
            ...
            keyword_elems = keywords_section.find_all('span', {'class': 'c-chapter-keyword'})
    ...
    async def _enrich_from_elsevier(self, session: aiohttp.ClientSession, 
                                   entry: Dict[str, Any], metadata: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if doi.startswith('10.1016/'):
                ...
                pii = self._doi_to_pii(doi)
                if pii:
                    sciencedirect_url = f"https://www.sciencedirect.com/science/article/abs/pii/{pii}"
                    metadata['publisher_url'] = sciencedirect_url
                    if self.verbose >= 2:
                        print(f"[ENRICH] Converted Elsevier DOI to ScienceDirect URL: {sciencedirect_url}")
                    
                    # Try to fetch abstract and keywords from ScienceDirect
                    await self._rate_limit('elsevier')
                    
                    try:
                        async with session.get(sciencedirect_url, timeout=self.config.get('preferences.timeout', 30)) as response:
                            if response.status == 200:
                                html = await response.text()
            ...
            search_url = f"https://www.sciencedirect.com/search/advanced?qs={quote_plus(title)}"
            ...
            async with session.get(search_url, timeout=self.config.get('preferences.timeout', 30)) as response:
                ...
                for result in results[:3]:  # Check first 3 results
                    ...
                    if self._calculate_similarity(title.lower(), result_title) > 0.8:
                        chapter_url = 'https://www.sciencedirect.com' + result.get('href', '')
                        metadata['publisher_url'] = chapter_url
                        
                        # Extract DOI if present
                        doi_elem = result.find_parent().find('span', {'class': 'doi'})
                        if doi_elem:
                            metadata['doi'] = doi_elem.get_text(strip=True).replace('DOI:', '').strip()
    ...
    async def _enrich_from_wiley(self, session: aiohttp.ClientSession, 
                                 entry: Dict[str, Any], metadata: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            async with session.get(search_url, timeout=self.config.get('preferences.timeout', 30)) as response:
                ...
                for result in results[:3]:
                    ...
                    if self._calculate_similarity(title.lower(), result_title) > 0.8:
                        chapter_url = 'https://onlinelibrary.wiley.com' + result.get('href', '')
                        metadata['publisher_url'] = chapter_url
    ...
    def _parse_wiley_page(self, html: str, metadata: Dict[str, Any]) -> None:
        ...
        # Extract abstract
        if not metadata.get('abstract'):
            abstract_section = soup.find('section', {'class': 'article-section__abstract'})
        ...
        if not metadata.get('keywords'):
            keywords = []
            keywords_section = soup.find('section', {'class': 'article-section__keywords'})
            if keywords_section:
    ...
    async def _enrich_from_scholar(self, session: aiohttp.ClientSession, 
                                   entry: Dict[str, Any], metadata: Dict[str, Any]) -> bool:
    ...
    async def _enrich_from_llm(self, session: aiohttp.ClientSession, 
                               entry: Dict[str, Any], metadata: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            async def search_academic_sources(query: str) -> List[Dict[str, Any]]:
                ...
                try:
                    ...
                    # Prioritize results: CrossRef > Semantic Scholar > Web Search
                    for source in ['crossref', 'semantic_scholar', 'unpaywall', 'web_search', 'direct_publisher']:
                        if source in all_results:
                            combined_results.extend(all_results[source][:3])  # Take top 3 from each
                    ...
                    if self.verbose >= 3:
                        print(f"[LLM_ENRICHER] Found {len(unique_results)} unique results from all sources")
            ...
            if success and self.verbose >= 2:
                ...
                if metadata.get('keywords'):
                    print(f"  - Keywords: {len(metadata['keywords'])} ({metadata.get('keyword_source', 'unknown')} source)")
    ...
    async def _fetch_from_publisher_url(self, session: aiohttp.ClientSession, metadata: Dict[str, Any]) -> None:
        ...
        try:
            ...
            async with session.get(url, timeout=self.config.get('preferences.timeout', 30)) as response:
                if response.status == 200:
                    ...
                    if self.verbose >= 3:
                        print(f"[ENRICH] Successfully fetched publisher page ({len(html)} bytes)")
                    ...
                    if self.verbose >= 2:
                        if metadata.get('abstract'):
                            print(f"[ENRICH] Found abstract: {len(metadata['abstract'])} chars")
                        else:
                            print("[ENRICH] No abstract found on publisher page")
                        if metadata.get('keywords'):
                            print(f"[ENRICH] Found {len(metadata['keywords'])} keywords from publisher")
                else:
                    if self.verbose >= 3:
                        print(f"[ENRICH] Failed to fetch publisher page: HTTP {response.status}")
        except Exception as e:
            if self.verbose >= 3:
                print(f"[ENRICH] Error fetching publisher page: {e}")
    
    def _parse_generic_publisher_page(self, html: str, metadata: Dict[str, Any]) -> None:
        ...
        if not metadata.get('abstract'):
            ...
            if abstract_meta:
                ...
                if abstract_text and len(abstract_text) > 100:  # Ensure it's not just a short description
                    metadata['abstract'] = abstract_text
        ...
        if not metadata.get('keywords'):
            ...
            # Try meta tags
            keyword_metas = soup.find_all('meta', {'name': re.compile(r'keywords?', re.I)}) + \
            ...
            for meta in keyword_metas:
                ...
                keywords.extend([k.strip() for k in kws if k.strip() and len(k.strip()) > 2])
